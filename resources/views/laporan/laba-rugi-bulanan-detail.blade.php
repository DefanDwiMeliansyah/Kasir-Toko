@extends('layouts.laporan', ['title' => 'Laporan Laba Rugi Bulanan Detail'])
@section('content')
<h1 class="text-center">Laporan Laba Rugi Bulanan - Detail</h1>
...
<!-- Tabel Detail Laba Rugi per Hari -->
<div class="mt-4">
    <h3>Detail Laba Rugi per Hari</h3>
    <div class="table-responsive">
        <table class="table table-bordered table-sm">
            ...
            <tbody>
                @forelse ($detailHarian as $key => $row)
                <tr>
                    <td>{{ $key + 1 }}</td>
                    ...
                    <td class="text-right {{ $row->laba >= 0 ? 'text-success' : 'text-danger' }}">
                        <strong>{{ number_format($row->laba, 0, ',', '.') }}</strong>
                    </td>
                    <td class="text-center {{ $row->margin_persen >= 0 ? 'text-success' : 'text-danger' }}">
                        <strong>{{ number_format($row->margin_persen, 2, ',', '.') }}%</strong>
                    </td>
                </tr>
                @empty
                <tr>
            ...
            @if ($detailHarian->count() > 0)
            <tfoot class="table-secondary">
                <tr>
                    <th colspan="2">Total Keseluruhan</th>
                    <th class="text-center">{{ number_format($detailHarian->sum('jumlah_transaksi'), 0, ',', '.') }}</th>
                    <th class="text-right">{{ number_format($detailHarian->sum('pendapatan'), 0, ',', '.') }}</th>
                    <th class="text-right">{{ number_format($detailHarian->sum('hpp'), 0, ',', '.') }}</th>
                    <th class="text-right {{ $detailHarian->sum('laba') >= 0 ? 'text-success' : 'text-danger' }}">
                        <strong>{{ number_format($detailHarian->sum('laba'), 0, ',', '.') }}</strong>
                    </th>
                    <th class="text-center {{ $ringkasan->margin_keseluruhan >= 0 ? 'text-success' : 'text-danger' }}">
                        <strong>{{ number_format($ringkasan->margin_keseluruhan, 2, ',', '.') }}%</strong>
                    </th>
            ...
            <table class="table table-bordered table-sm">
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="text-right">{{ number_format($ringkasan->total_transaksi, 0, ',', '.') }}</td>
                    </tr>
                    <tr>
...
<div class="mt-4">
    <small class="text-muted">
        ...
        * Data hanya menampilkan transaksi yang tidak dibatalkan<br>
